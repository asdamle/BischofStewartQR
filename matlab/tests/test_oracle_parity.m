...
for idx = 1:numel(zoo)
    ...
    for b = 1:numel(backends)
        ...
        sv = svd(R(1:kk, 1:kk));          % ||R11^{-1}|| via svd (no inv): the
        % Frobenius norm is norm(1./sv), the spectral norm is max(1./sv) = 1/min(sv).
        verifyLessThanOrEqual(testCase, norm(1 ./ sv), ...
            sqrt(kk * (n - kk + 1)) * (1 + 1e-12), ...
            sprintf('%s/%s: Frobenius Osinsky bound', z.name, backends{b}));
        verifyLessThanOrEqual(testCase, max(1 ./ sv), ...
