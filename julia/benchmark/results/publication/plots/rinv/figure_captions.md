...
Run: 20260622_161419; seeds: 5; BLAS threads: 1, 4. In all figures, BSQR + $R_{11}^{-1}R_{12}$ is compared against CPQR + $R_{11}^{-1}R_{12}$; both timed paths materialize Q, R, and the permutation.
...
**fig_square_runtime.** Median runtime versus matrix size for square matrices (log–log). Lines are geometric means over 5 seeds; faint bands show the range across seeds. Panels: matrix family × BLAS threads.
